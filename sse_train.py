...
import random
import pickle
from sse_model import *
import torch.nn as nn
from torch import optim
from sse_util import *
...
# This function padding the sentence to same length
# and map the word to ID to create input tensor
...
#   assert pretrained_emb.shape == (len(tokens), 300)  
  ...
  for word in all_words:
    if word in word2vec:
      ...
      pretrained_emb.append(np.random.uniform(-0.05, 0.05, size=[200])) 
  embeddings = np.stack(pretrained_emb)

  #TODO: It would be better to use library to split the data
  ...
  '''
  The make batch function may not be necessay 
  '''
