...
from dec_att_model import *
import argparse
import sys
from dec_att_util import *
import time
import torch
import random
import logging
...
def create_batch(data,from_index, to_index):
	...
	#print(lsent)
	left_sents = torch.cat((dict[ ].view(1, -1) for word in lsent))
...
if __name__ == '__main__':
	...
	model=DecAtt(200, 2, len(tokens),EMBEDDING_DIM, PROJECTED_EMBEDDING_DIM, pretrained_emb)
	if torch.cuda.is_available():
		model = model.cuda()
